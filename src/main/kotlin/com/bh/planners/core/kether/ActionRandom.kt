 ...
 */
class ActionRandom(val from: ParsedAction<*>, val to: ParsedAction<*>, val action: ParsedAction<*>? = null) :
    ScriptAction<Any?>() {
            ...
            frame.newFrame(action).run<Any?>().thenAcceptAsync({
                when (it) {
                    ...
                    is Array<*> -> {
                        ...
                        future.complete(
                            if (it.isInt()) random(Coerce.toInteger(it)) else random(
                                0.0,
                                Coerce.toDouble(it)
                            )
                        )
    ...
    internal object Parser {
         ...
         */
        @KetherParser(["random", "random2"], namespace = NAMESPACE, shared = true)
