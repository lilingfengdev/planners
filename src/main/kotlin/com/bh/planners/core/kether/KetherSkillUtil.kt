    ...
    if (of != null) {
        this.run(of).str { skill ->
            future.complete(
                func(
                    this,
                    this.bukkitPlayer()?.plannersProfile?.getSkill(skill) ?: error("No skill $skill")
                )
            )
        }
    } else {
...
fun actionSkillFuture(of: ParsedAction<*>?, func: QuestContext.Frame.(PlayerJob.Skill) -> CompletableFuture<*>) =
    actionFuture { future ->
        if (of != null) {
            run(of).str { skill ->
                func(
...
fun actionProfileNow(action: ParsedAction<*>, func: QuestContext.Frame.(value: Any?, profile: PlayerProfile) -> Any?) =
    actionFuture { future ->
        val player = this.bukkitPlayer() ?: error("No player selected.")
        if (!player.plannersProfileIsLoaded) {
            future.complete("__LOADED__")
...
fun actionProfileTake(action: ParsedAction<*>, func: QuestContext.Frame.(value: Any, profile: PlayerProfile) -> Unit) =
    actionTake {
        val player = this.bukkitPlayer() ?: error("No player selected.")
        if (!player.plannersProfileIsLoaded) {
            return@actionTake CompletableFuture.completedFuture(null)
